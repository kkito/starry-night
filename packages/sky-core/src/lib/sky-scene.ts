// packages/sky-core/src/lib/sky-scene.ts
// 全量绘制一帧 3D 天穹（与 web SkyHtml3D 同源，供小程序 SkyCanvas3D 复用）。
// 纯函数：只依赖传入的 CanvasRenderingContext2D 与包内纯函数，无 DOM 访问。
import { pointSizeFor } from './dome';
import { COLORS } from './tokens';
import { projectHtml3D, silhouetteShapes, silhouettePx, directionLabels } from './sky-html3d';
import type { DrawStar } from './drawlist';
import type { StarTrack } from './track';
...
/** 全量绘制一帧（天穹渐变/辉光/仰角圈/经线/方位标注/剪影/轨迹/星点）。

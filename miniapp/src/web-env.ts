...
export function getViewport(): { width: number; height: number; pixelRatio: number } {
  ...
  const win: any = typeof T.getWindowInfo === 'function' ? T.getWindowInfo() : {};
  const dev: any = typeof T.getDeviceInfo === 'function' ? T.getDeviceInfo() : {};
  const legacy: any = typeof Taro.getSystemInfoSync === 'function' ? Taro.getSystemInfoSync() : {};
